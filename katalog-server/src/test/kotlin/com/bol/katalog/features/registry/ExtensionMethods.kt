...
fun Schema.create() = CreateSchemaCommand(namespace.id, id, name, type)
fun Schema.created() = SchemaCreatedEvent(namespace.id, id, name, type)
fun Schema.delete() = DeleteSchemaCommand(id)
fun Schema.deleted() = SchemaDeletedEvent(id)
fun Version.create() = CreateVersionCommand(schema.id, id, semVer.toString())
fun Version.created() = VersionCreatedEvent(schema.id, id, semVer.toString())
fun Version.delete() = DeleteVersionCommand(id)
fun Version.deleted() = VersionDeletedEvent(id)
fun Artifact.create(data: ByteArray) = CreateArtifactCommand(version.id, id, filename, mediaType, data)
fun Artifact.created(data: ByteArray) = ArtifactCreatedEvent(version.id, id, filename, mediaType, data)
