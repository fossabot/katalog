...
class VersionAggregateTest {
    ...
    fun `Can register versions`() {
        runBlocking {
            expectThat(registry.getVersions("id-ns1-schema1")).containsExactly(
                Version(
                    "id-ns1-schema1-v100",
            ...
            expectThat(registry.getVersions("id-ns2-schema3")).containsExactly(
                Version(
                    "id-ns2-schema3-v100",
                    TestData.clock.instant(),
                    Semver("1.0.0", Semver.SemverType.IVY)
    ...
    fun `Can find schemas of versions`() {
        runBlocking {
            expectThat(registry.getVersions("id-ns1-schema1").map { registry.getVersionSchemaId(it.id) }.distinct().single()).isEqualTo(
                "id-ns1-schema1"
            )
            expectThat(registry.getVersions("id-ns2-schema3").map { registry.getVersionSchemaId(it.id) }.distinct().single()).isEqualTo(
                "id-ns2-schema3"
            )
        }
    }
    ...
    fun `Can delete version`() {
        runBlocking {
            val version = registry.getVersion("id-ns1-schema1-v100")
            ...
            expectThat(catching { registry.getVersionSchemaId(version.id) }).throws<NotFoundException>()
        }
    }
    ...
    fun `Can get current major versions`() {
        runBlocking {
            val result = registry.getCurrentMajorVersions(listOf(v("1.0.0"), v("1.0.1"), v("2.0.0-SNAPSHOT")))
