...
class InMemoryEventStoreTest {
    ...
    fun `Can roundtrip events`() {
        runBlocking {
            ...
            assertThat(data).containsExactly(Event(metadata, mapOf("foo" to "1")), Event(metadata, mapOf("foo" to "2")))

            val (data2, _) = eventStore.get(EventQuery(cursor = cursor))
            assertThat(data2).containsExactly(Event(metadata, mapOf("foo" to "3")))
        }
    }
}
